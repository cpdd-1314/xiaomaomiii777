...
(function () {
  ...
  // 噪声爆破（烟花炸响 / 礼花 pop / 雷声 / 火焰噼啪）
  function _noise(start, dur, opt) {
    opt = opt || {};
    const t0 = Math.max(0, start); // 防止起播附近出现负时间戳
    const src = audioCtx.createBufferSource();
    src.buffer = getNoiseBuffer();
    const f = audioCtx.createBiquadFilter();
    f.type = opt.type || "lowpass"; f.frequency.value = opt.freq || 1000; f.Q.value = opt.q || 1;
    const g = audioCtx.createGain();
    const gain = opt.gain != null ? opt.gain : 0.3;
    g.gain.setValueAtTime(0.0001, t0);
    g.gain.linearRampToValueAtTime(gain, t0 + (opt.attack || 0.005));
    g.gain.exponentialRampToValueAtTime(0.0001, t0 + dur);
    src.connect(f); f.connect(g); g.connect(masterNode);
    src.start(t0); src.stop(t0 + dur + 0.05);
    activeNodes.push(src);
  }
  // 频率滑音（时空跃迁 / 火焰轰鸣 / 雷声闷响）
  function _sweep(start, dur, f0, f1, type, gain) {
  ...
  // 持续噪声床（火焰轰鸣 / 雨声底噪）
  function _bed(type, freq, gain, dur) {
    const src = audioCtx.createBufferSource();
    src.buffer = getNoiseBuffer(); src.loop = true;
    const f = audioCtx.createBiquadFilter(); f.type = type; f.frequency.value = freq;
    const g = audioCtx.createGain(); g.gain.value = gain;
    src.connect(f); f.connect(g); g.connect(masterNode);
    src.start(audioCtx.currentTime); src.stop(audioCtx.currentTime + dur + 0.1);
    activeNodes.push(src);
  }
  // 持续音垫（极光空灵和声）
  function _pad(freq, gain, dur, start) {
    const g = audioCtx.createGain();
    g.gain.setValueAtTime(0.0001, start);
    g.gain.linearRampToValueAtTime(gain, start + 1.5);
    g.gain.setValueAtTime(gain, start + dur - 1.5);
    g.gain.exponentialRampToValueAtTime(0.0001, start + dur);
  ...
  // ============================================================
  // 每个特效的专属音效（纯 Web Audio 合成，零依赖、真正录进视频）
  // ============================================================
  const effectSounds = {
    // 🎆 烟花盛典：升空 whoosh + 炸响 boom + 欢快琶音
    fireworks(start) {
      const chords = [[523.25, 659.25, 783.99], [587.33, 739.99, 880], [659.25, 830.61, 987.77], [698.46, 880, 1046.5]];
      const beat = 0.5;
      for (let i = 0; i < DURATION / beat; i++) {
        chords[i % chords.length].forEach((f, idx) => _note(masterNode, f, start + i * beat + idx * 0.03, beat * 0.8, "triangle", 0.11));
      }
      for (let i = 0; i < 8; i++) {
        const bt = start + 0.3 + i * 0.9 + Math.random() * 0.3;
        _noise(bt - 0.5, 0.5, { type: "bandpass", freq: 1200, q: 1.5, gain: 0.10 });   // 升空
        _sweep(bt, 0.45, 170, 50, "sine", 0.32);                                        // 炸响低频
        _noise(bt, 0.4, { type: "lowpass", freq: 400, gain: 0.28 });                     // 炸响噪声
      }
    },
    // ✨ 星河璀璨：高音铃铛闪烁（五声音阶）
    starlight(start) {
      ...
      for (let i = 0; i < 24; i++) {
        const t = start + i * 0.33 + Math.random() * 0.12;
        _note(masterNode, scale[Math.floor(Math.random() * scale.length)] * 2, t, 0.5, "sine", 0.10);
      }
    },
    // 🌈 霓虹脉冲：电子贝斯脉冲 + 方波琶音
    neon(start) {
      const bass = [110, 110, 146.83, 110, 130.81, 110, 146.83, 164.81];
      const arp = [440, 554.37, 659.25, 880, 554.37, 659.25, 880, 1108.73];
      const beat = 0.25;
      for (let i = 0; i < DURATION / beat; i++) {
        _note(masterNode, bass[i % bass.length] / 2, start + i * beat, beat * 0.9, "sawtooth", 0.10);
        _note(masterNode, arp[i % arp.length], start + i * beat, beat * 0.5, "square", 0.05);
      }
    },
    // 💫 粒子汇聚：上升 riser + 浮现时长和弦（科幻感）
    particles(start) {
      const steps = [261.63, 329.63, 392, 523.25, 659.25, 783.99, 1046.5, 1318.5];
      for (let i = 0; i < steps.length; i++) _note(masterNode, steps[i], start + i * 0.28, 0.4, "sine", 0.09);
      _sweep(start, 2.4, 200, 2200, "sine", 0.07);
      [523.25, 659.25, 783.99, 1046.5].forEach((f, idx) => _note(masterNode, f, start + 2.6 + idx * 0.02, 3, "triangle", 0.13));
    },
    // 🪙 金粉富贵：喜庆主调 + 金属叮当
    gold(start) {
      const motif = [659.25, 783.99, 880, 1046.5, 880, 783.99, 659.25, 987.77];
      const beat = 0.5;
      for (let i = 0; i < DURATION / beat; i++) {
        _note(masterNode, motif[i % motif.length], start + i * beat, beat * 0.7, "triangle", 0.13);
        _note(masterNode, motif[i % motif.length] * 2, start + i * beat + 0.12, 0.25, "square", 0.045); // 金币叮当
      }
    },
    // 💖 爱心满天：八音盒般的大七和弦（浪漫）
    hearts(start) {
      const prog = [[523.25, 659.25, 783.99, 987.77], [587.33, 698.46, 880, 1108.73], [493.88, 622.25, 739.99, 932.33], [523.25, 659.25, 783.99, 987.77]];
      const beat = 1;
      for (let i = 0; i < DURATION / beat; i++) {
        prog[i % prog.length].forEach((f, idx) => _note(masterNode, f, start + i * beat + idx * 0.06, beat * 0.9, "sine", 0.09));
      }
    },
    // 🔥 烈焰升腾：低频火焰轰鸣 + 噼啪
    fire(start) {
      _bed("lowpass", 380, 0.10, DURATION);          // 持续轰鸣
      _sweep(start, DURATION, 60, 55, "sine", 0.11); // 低频隆隆
      for (let i = 0; i < 32; i++) _noise(start + Math.random() * DURATION, 0.05, { type: "highpass", freq: 3000, gain: 0.05 }); // 噼啪
    },
    // 🚀 时空隧道：多次上升 whoosh（跃迁感）
    warp(start) {
      for (let i = 0; i < 5; i++) {
        const t = start + i * 1.5;
        _sweep(t, 1.3, 120, 1800, "sawtooth", 0.11);
        _noise(t, 1.3, { type: "bandpass", freq: 1500, q: 0.7, gain: 0.09 });
      }
    },
    // 🎊 礼花满天：欢快琶音 + pop 爆破
    confetti(start) {
      ...
      const beat = 0.22;
      for (let i = 0; i < DURATION / beat; i++) _note(masterNode, scale[i % scale.length], start + i * beat, beat * 0.7, "triangle", 0.11);
      for (let i = 0; i < 16; i++) _noise(start + Math.random() * DURATION, 0.08, { type: "bandpass", freq: 2500, q: 0.8, gain: 0.13 }); // pop
    },
    // 👑 皇冠加冕：小号般加冕号角（fanfare）
    crown(start) {
      const fanfare = [
        [0, 523.25, 0.25], [0.28, 523.25, 0.25], [0.56, 523.25, 0.25], [0.84, 783.99, 0.6],
        [1.6, 880, 0.25], [1.88, 880, 0.25], [2.16, 1046.5, 0.25], [2.44, 1046.5, 0.7],
      ];
      const play = (base, gain) => fanfare.forEach(([dt, f, dur]) => {
        _note(masterNode, f, base + dt, dur, "sawtooth", gain);
        _note(masterNode, f / 2, base + dt, dur, "sawtooth", gain * 0.5); // 加厚低八度
      });
      play(start, 0.15);
      play(start + 4.2, 0.13);
    },
    // 🌌 极光盛宴：空灵持续音垫 + 微光铃
    aurora(start) {
      [261.63, 329.63, 392, 523.25].forEach((f) => _pad(f, 0.055, DURATION, start));
      for (let i = 0; i < 16; i++) _note(masterNode, 1046.5 * (1 + Math.floor(Math.random() * 3)), start + 0.5 + i * 0.5, 0.6, "sine", 0.045);
    },
    // ⚡ 雷霆万钧：雨声底噪 + 落雷闷响（与画面闪电同步）
    thunder(start) {
      _bed("highpass", 4000, 0.025, DURATION);       // 雨声底噪
      for (let i = 0; i < 6; i++) {
        const bt = start + 0.4 + i * 1.2;
        _sweep(bt, 0.9, 90, 40, "sine", 0.28);        // 落雷低频
        _noise(bt, 0.9, { type: "lowpass", freq: 300, gain: 0.32 });
        _noise(bt + 0.05, 0.6, { type: "bandpass", freq: 800, q: 0.5, gain: 0.13 });
      }
    },
  };

  function playEffectSound(effect, start) {
    const fn = effectSounds[effect] || effectSounds.fireworks;
    fn(start);
  }
  function stopAudio() {
  ...
  function startAudio() {
    ...
    if (state.bgmBuffer) {
      ...
      playEffectSound(state.effect, now);
    }
  }
  ...
  // 视频生成
  // ============================================================
  // 选择录制编码：若目标为 MP4 且浏览器原生支持（iOS/微信），直接录 MP4；
  // 否则（桌面 Chrome/Android）录 webm，后续再按需转 MP4。
  function pickMime() {
    const cands = ["video/webm;codecs=vp9", "video/webm;codecs=vp8", "video/webm"];
    if (state.format === "mp4") {
      const mp4s = ["video/mp4;codecs=avc1.42E01E", "video/mp4;codecs=h264", "video/mp4"];
      for (const m of mp4s) if (window.MediaRecorder && MediaRecorder.isTypeSupported(m)) return m;
    }
  ...
  // ---------- MP4 导出 ----------
  // 双路径策略（兼顾兼容性与零依赖）：
  //  1) iOS Safari / 微信内置浏览器：MediaRecorder 原生支持 video/mp4，直接录制即可，无需任何依赖。
  //  2) 桌面 Chrome / Android：原生只支持 webm；若页面经由「跨源隔离」环境打开
  //     （即用 serve.py 启动的本地服务器，带 COOP/COEP 头），则用 ffmpeg.wasm 0.12
  //     把 webm 转成 mp4；否则自动回退为 webm 并给出提示。
  // 说明：ffmpeg.wasm 的多线程核心依赖 SharedArrayBuffer / 跨源隔离，file:// 双击打开
  //       不满足该条件，必须配合 serve.py 使用（见仓库内 serve.py 说明）。
  let ffmpeg = null, ffmpegLoading = null, ffmpegReady = false, mp4ProgressCb = null;
  ...
  async function ensureFFmpeg() {
    ...
    if (!ffmpegLoading) {
      ffmpegLoading = (async () => {
        ...
        core.on("progress", (e) => { if (mp4ProgressCb) mp4ProgressCb(clamp(e.progress || 0, 0, 1)); });
        await core.load({
  ...
  async function generateVideo() {
    ...
    let ext = blob.type.indexOf("mp4") >= 0 ? "mp4" : "webm";
    if (state.format === "mp4" && ext !== "mp4") {
      // 录到的是 webm，需要转换
      if (self.crossOriginIsolated) {
        try {
          setStatus("正在转换 MP4（首次需联网加载转换器，请稍候）…", "busy");
          blob = await convertToMp4(blob, (p) => setStatus("正在转换 MP4… " + Math.round(p * 100) + "%", "busy"));
          ext = "mp4";
        } catch (err) {
          console.error("MP4 转换失败：", err);
          setStatus("⚠️ MP4 转换失败（可能无网络），已回退为 WEBM，仍可正常下载", "");
        }
      } else {
        setStatus("⚠️ 当前打开方式不支持导出 MP4（需跨源隔离）。请用 Safari / 微信打开，或运行 python3 serve.py 后用 http://localhost 打开本页。已为你导出兼容的 WEBM。", "");
      }
    }
    const url = URL.createObjectURL(blob);
    resultVideo.src = url;
    downloadLink.href = url;
    downloadLink.download = "宝子官宣特效_" + state.effect + "." + ext;
    resultWrap.classList.remove("hidden");
    state.recording = false; genBtn.disabled = false; playBtn.disabled = false;
    const sizeKB = (blob.size / 1024).toFixed(0);
    const withSound = state.soundOn ? "含音效 " : "无声 ";
    setStatus("✅ 视频生成完成（" + withSound + sizeKB + " KB · " + ext.toUpperCase() + "），可预览并下载", "ok");
  }
